...
/*
	...
	For STOMP 1.0 clients:  if an HK_ID header is supplied, attempt to use it.  If the
	HK_ID header is not unique, return an error.  If no "id" header is supplied, send the
	SUBSCRIBE frame without an HK_ID header.  Some brokers may respond with an ERROR
	frame in this case if the subscription is seen as a duplicate.

	For STOMP 1.1+ clients: If any client does not supply an HK_ID header, attempt to generate
	a unique HK_ID.  In all cases, do not allow duplicate subscription "id"s in this session.
